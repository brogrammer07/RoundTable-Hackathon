...
import { addManager } from "../../../redux/actions/adminActions";
import Select from "@mui/material/Select";
import MenuItem from "@mui/material/MenuItem";
import Spinner from "../../../utils/Spinner";
import { ADD_MANAGER, SET_ERRORS } from "../../../redux/actionTypes";
...
const Body = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    dispatch(addManager(value));
    setError({});
    setLoading(true);
  };

  useEffect(() => {
    if (store.errors || store.admin.managerAdded) {
      setLoading(false);
      if (store.admin.managerAdded) {
        setValue({
          name: "",
          dob: "",
          email: "",
        ...
        dispatch({ type: ADD_MANAGER, payload: false });
      }
    } else {
      setLoading(true);
    }
  }, [store.errors, store.admin.managerAdded]);
